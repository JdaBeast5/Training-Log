...
test('sabotage-relevant: exactly 8 real aiKeySetupPrompt( call sites (the original 7, plus Snap a Meal newly converted from hand-rolled HTML)', (assert)=>{
  const count = countCallSites(src, 'aiKeySetupPrompt');
  assert.strictEqual(count, 8, `expected 8 real call sites, found ${count}`);
});
