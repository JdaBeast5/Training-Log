...
test('sabotage-relevant: the real total count of converted sites matches what this pass actually touched (112 white, 60 black) — not a partial sweep that happens to pass the spot-checks above', (assert)=>{
  ...
  assert.strictEqual(whiteCount, 112, 'the real total of rgba(var(--surface-white-rgb), occurrences in the file must be 112 — every white-alpha CSS declaration in the <style> block, and no more (the :root declaration line itself is `--surface-white-rgb:255,255,255;`, which has no rgba( prefix and so is not counted here)');
