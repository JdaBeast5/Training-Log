...
Branch: `claude/supplement-recommendations-vi9hw2` (restarted fresh from `origin/Branch-1` at the start of this session — the branch had been merged via PR #22 / `b8e1f2f`). `APP_VERSION`/`VERSION` bumped 3.258 → **3.260**. No `SCHEMA_VERSION` change — no stored-data shape changed.
...
Also mid-session, PR #23 (`claude/supplement-descriptions-expand`, "Fix description persistence, make My Supplements collapsible") merged into `Branch-1` and claimed v3.259 first — this session's branch was rebased onto that new tip (fast-forward, no functional conflicts — the only conflict was both sessions independently claiming the same next `WHATS_NEW` version key) and re-bumped to 3.260 so the two shipped versions don't collide.
